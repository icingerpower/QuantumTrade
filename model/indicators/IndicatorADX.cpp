...
double IndicatorADX::compute(
    ...
    int hi, int lo, int cl, int vol,
    const Tick*, const QMap<QString, QVariant>& params) const
{
    ...
    {
        double H  = q[i][hi],   Hprev = q[i-1][hi];
        double L  = q[i][lo],   Lprev = q[i-1][lo];
        double Cprev = q[i-1][cl];
    ...
    auto wilder = [&](const std::vector<double>& src)
    {
        ...
        out[period-1] = sum;
        // then: out[i] = out[i-1] - (out[i-1]/period) + src[i]
    ...
    // 3) Compute DI+ and DI- and DX
    std::vector<double> DX(sTR.size());
    ...
    // 4) Smooth DX into ADX via same Wilder smoothing
    auto adxSeries = wilder(DX);
    return adxSeries.back() / period;
    // note: we divide initial sum by period to get the rolling average
}
